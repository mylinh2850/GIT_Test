    ...
    switch (statusUartReceive)
    {
        case WAIT_BYTE1:
            if (tempReceive == 04)
                ...
                statusUartReceive = WAIT_BYTE1;
            break;
        case WAIT_BYTE2:
            if (tempReceive == 03)
                statusUartReceive = WAIT_BYTE3;
            else
                statusUartReceive = WAIT_BYTE1;
            break;
        case WAIT_BYTE3:
            if (tempReceive == 05)
                statusUartReceive = WAIT_BYTE4;
            else
                statusUartReceive = WAIT_BYTE1;
            break;
        case WAIT_BYTE4:
